...
impl Registers {
    ...
    pub fn get16(&self, rr: Reg16) -> u16 {
        self.data[rr as usize + 1] as u16 + ((self.data[rr as usize] as u16) << 8)
    }
    ...
    pub(crate) fn swap(&mut self, rr: Reg16) {
        let ih = rr as usize;
        std::mem::swap(&mut self.data[ih], &mut self.shadow[ih]);

        let il = rr as usize + 1;
        std::mem::swap(&mut self.data[il], &mut self.shadow[il]);
    }
