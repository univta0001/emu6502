...
impl Video {
    ...
    pub fn new(mem: Rc<RefCell<Mmu>>) -> Self {
    ...
    fn read_raw_text_memory(&self, addr: usize) -> u8 {
        let mmu = self.mem.borrow();
        if self.video_page2 {
            if !mmu._80storeon {
                mmu.mem_read(addr as u16 + 0x800)
            } else {
                mmu.mem_read(addr as u16 + 0x400)
            }
        } else {
            mmu.mem_read(addr as u16 + 0x400)
        }
    ...
    fn read_raw_aux_text_memory(&self, addr: usize) -> u8 {
        ...
        if self.video_page2 {
            if !mmu._80storeon {
                mmu.mem_aux_read(addr as u16 + 0x800)
            } else {
                mmu.mem_aux_read(addr as u16 + 0x400)
            }
        } else {
            mmu.mem_aux_read(addr as u16 + 0x400)
        }
    ...
    fn read_raw_hires_memory(&self, addr: usize) -> u8 {
        ...
        if self.video_page2 {
            if !mmu._80storeon {
                mmu.mem_read(addr as u16 + 0x4000)
            } else {
                mmu.mem_read(addr as u16 + 0x2000)
            }
        } else {
            mmu.mem_read(addr as u16 + 0x2000)
        }
    ...
    fn read_raw_aux_hires_memory(&self, addr: usize) -> u8 {
        ...
        if self.video_page2 {
            if !mmu._80storeon {
                mmu.mem_aux_read(addr as u16 + 0x4000)
            } else {
                mmu.mem_aux_read(addr as u16 + 0x2000)
            }
        } else {
            mmu.mem_aux_read(addr as u16 + 0x2000)
        }
